...
> **Docker 镜像已切换到 Docker Hub**（2026-07 起替代已损坏的 GHCR）。  
> `docker-compose.yml` 中 `image: cwtchtome/takeout-*` 指向 Docker Hub，默认自动拉取最新镜像。
...
> **镜像仓库**：已切换到 **Docker Hub**（`cwtchtome/takeout-*`），所有镜像已推送完毕。
>
...
git commit -m "改了xxx"
git push
# CI 自动在 GitHub 服务器上构建镜像并推送到 Docker Hub
```
...
| `docker compose pull`            | 拉取 Docker Hub 最新镜像     |
...
2. 登录 Docker Hub
3. 构建 4 个 Docker 镜像
4. 推送到 Docker Hub（`cwtchtome/takeout-*`）

**最终效果**：代码推送到 GitHub 后，镜像自动更新到 Docker Hub，任何人 `docker compose pull` 即可获取最新版本。
...
> **注意**：推送 Docker Hub 需要在 GitHub 仓库设置中添加 `DOCKER_USERNAME` 和 `DOCKER_TOKEN` 两个 Secrets（Docker Hub 账号的用户名和 Access Token）。
      ...
      - name: Build & push backend
        ...
        with:
          ...
          tags: |
            ...
            cwtchtome/takeout-backend:${{ github.sha }}
...
```
backend    ─── checkout → 登录 Docker Hub → Maven 编译 → 构建镜像 → push
h5         ─── checkout → 登录 Docker Hub → npm build  → 构建镜像 → push     ← 同时进行
merchant   ─── checkout → 登录 Docker Hub → npm build  → 构建镜像 → push
admin      ─── checkout → 登录 Docker Hub → npm build  → 构建镜像 → push
```
...
> **当前推送到 Docker Hub**（2026-07 起临时替代已损坏的 GHCR）。  
...
GitHub Actions 检测到代码改动
 （路径匹配才触发）
       │
       ├─ backend：    JDK 17 + Maven → mvn package → JRE 17 镜像 → push 到 Docker Hub
       ├─ h5：         Node 20 → npm ci → npm run build → Nginx 镜像 → push 到 Docker Hub
       ├─ merchant：   Node 20 → npm ci → npm run build → Nginx 镜像 → push 到 Docker Hub
       └─ admin：      Node 20 → npm ci → npm run build → Nginx 镜像 → push 到 Docker Hub
       │
       ▼
...
- **Docker Hub 镜像列表**：https://hub.docker.com/u/cwtchtome
